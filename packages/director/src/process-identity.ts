...
function windowsOrphanScanScript(quotedNonce: string, holderPid: number): string {
  return [
    ...
    "  # Every parentless CIM row is emitted. Interpret is floor-gated and",
    "  # nonce-gated: a parentless other-nonce row after the floor is",
    "  # UNAVAILABLE; an ordinary service created before the floor is not.",
    "  # Requiring -not $nonceReadable here drops the double-fork leaf whose",
    "  # environment was read, so that interpret rule never fires.",
    "  if ($n -eq $target -or $isDesc -or (-not $parentPresent)) {",
