...
import {
  ...
  WORKSPACE_REGISTRY_MIGRATION, createEmptyStateV1, migrateStateV1, validateStateV1,
} from "../src/index.js";
...
test("every pre-workspace record migrates to PERSONAL with nothing else changed", () => {
  ...
  // Two migrations run against a pre-workspace state: records gain a workspace, and the two
  // literal workspaces become the registry that business and brand workspaces later join. The
  // relationship promotion records nothing here because this fixture holds no relationships.
  assert.deepEqual(state.migrations.map((entry) => entry.migration), [WORKSPACE_MIGRATION, WORKSPACE_REGISTRY_MIGRATION]);
  assert.deepEqual(state.workspaces.map((entry) => entry.id), ["personal", "work"]);
  assert.equal(state.workspaces.every((entry) => entry.builtIn), true, "the registry is seeded with exactly the two built-ins");
});
...
test("identifiers, provenance, history, timestamps and relationships survive the migration exactly", () => {
  ...
  // The migrations legitimately add these; compare everything else byte for byte.
  delete (actual as { migrations?: unknown }).migrations;
  delete (expected as { migrations?: unknown }).migrations;
  delete (actual as { workspaces?: unknown }).workspaces;
  delete (actual as { relationships?: unknown }).relationships;
  ...
  assert.deepEqual(actual, expected, "the migrations add a workspace and the registry, and change nothing else");
