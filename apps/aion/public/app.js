...
const short = (value, length = 16) => `${String(value ?? "").slice(0, length)}â€¦`;
...
${cards(scoped(s.conversations), (c) => `<article class="card"><h2>${esc(c.title)}</h2><p class="meta">${esc(c.state)} Â· ${c.messages.length} messages Â· memory context ${c.memoryContextEnabled ? "on" : "off"} Â· updated ${esc(c.updatedAt)}</p>
<div class="thread">${c.messages.slice(-8).map((m) => `<p class="msg ${esc(m.role)}"><b>${esc(m.role)}${m.providerId ? ` Â· ${esc(m.providerId)}` : ""}:</b> ${esc(m.content)}</p>`).join("")}${streaming && openConversation === c.id ? `<p class="msg assistant streaming"><b>assistant:</b> ${esc(streaming)}<span class="cursor">â–</span></p>` : ""}</div>
...
<p class="meta">${esc(t.priority)} Â· ${esc(t.state)} Â· ${esc(t.tags.join(", ") || "no tags")} Â· from ${esc(t.provenance.sourceType)} Â· ${t.history.length} history entries</p>
...
<p class="meta">${r.enabled ? "enabled" : "disabled"} Â· every ${r.intervalMinutes} min Â· next ${esc(r.nextRunAt ?? "not scheduled")} Â· last ${esc(r.lastRunAt ?? "never")} Â· ${r.history.length} history entries</p>
...
${cards(scoped(s.memories), (m) => `<article class="card ${m.conflict === "conflicting" ? "conflict" : ""}"><h2>${esc(m.category)}${m.conflict === "conflicting" ? " Â· conflicting" : ""}</h2><p>${esc(m.content)}</p>
<p class="meta">${esc(m.confirmation)} Â· ${m.enabled ? "enabled" : "disabled"} Â· source ${esc(m.provenance.sourceType)}/${esc(m.provenance.sourceRef)} Â· recorded ${esc(m.sourceTimestamp)} Â· ${m.corrections.length} corrections</p>
...
${m.corrections.length ? `<details><summary>Correction history</summary>${m.corrections.map((c) => `<p class="meta">${esc(c.at)}: â€œ${esc(c.previousContent)}â€ â†’ â€œ${esc(c.correctedContent)}â€ (${esc(c.reason)})</p>`).join("")}</details>` : ""}
...
${cards(scoped(s.plans), (p) => `<article class="card"><h2>${esc(p.goal)}</h2><p class="meta">${esc(p.status)} Â· ${p.steps.length} steps Â· from ${esc(p.provenance.sourceType)} Â· ${esc(p.createdAt)}</p>
<ol>${p.steps.map((x) => `<li>${esc(x.title)} â€” ${esc(x.status)}${x.dependencies.length ? ` Â· depends on ${x.dependencies.length} earlier step(s)` : ""}${x.approvalRequired ? " Â· approval required" : ""}${x.requiredCapabilities.length ? ` Â· needs ${esc(x.requiredCapabilities.join(", "))}` : ""}${x.blockedReason ? ` Â· blocked: ${esc(x.blockedReason)}` : ""}<br><span class="meta">expects ${esc(x.expectedOutput)}</span></li>`).join("")}</ol>
...
function verifyArea(s) {
  ...
  return `<h1>Verify</h1><p class="lead">AION runs these itself. Each one is chosen from a fixed allowlist written in the repository â€” there is no command box, and nothing a model says can become part of a command.</p>
<p class="hint">This is how you get â€œrun the tests and tell me what failedâ€ without giving a developer agent a shell or write access: AION produces the evidence, then a <em>read-only</em> agent analyses it.</p>
${operations.length ? `<form data-form="verify"><label>Operation<select name="operationId">${operations.map((o) => `<option value="${esc(o.id)}">${esc(o.label)} â€” ${esc(o.displayCommand)}</option>`).join("")}</select></label>
<p class="meta">${operations.map((o) => `<code>${esc(o.displayCommand)}</code>`).join(" Â· ")} â€” all read-only.</p><button>Propose verification</button></form>` : `<div class="empty">No verification tooling is available on this computer.</div>`}
${cards(runs, (r) => `<article class="card"><h2>${esc(r.operationId)} Â· ${esc(r.outcome)}</h2>
<p class="meta"><code>${esc(r.displayCommand)}</code> Â· exit ${r.exitCode}${r.timedOut ? " Â· timed out" : ""} Â· ${r.durationMs} ms Â· ${esc(r.startedAt)} Â· digest ${esc(short(r.resultDigest))}${r.truncated ? " Â· output truncated" : ""}</p>
...
<ul>${capabilities.map((c) => `<li><code>${esc(c.id)}</code> â€” ${esc(c.privacy)} Â· approval ${esc(c.approval)} Â· timeout ${c.timeoutMs} ms Â· ${c.maxRetries} retries</li>`).join("")}</ul></div>
...
<label>Task boundary<select name="mode"><option value="read-only">read-only â€” the agent may read this repository but not change it</option><option value="workspace-write">workspace-write â€” the agent may modify files inside this repository</option></select></label>
<p class="meta">Selected bridge: ${esc(model.developerBridge.displayName)} â€” ${esc(model.developerBridge.detail)}${model.developerBridge.available ? "" : " Proposals will be rejected at execution."}</p>
...
<p class="meta">${esc(a.state)} Â· digest ${esc(short(a.inputDigest))} Â· requested ${esc(a.requestedAt)} Â· expires ${esc(a.expiresAt)}${a.decidedAt ? ` Â· decided ${esc(a.decidedAt)}` : ""}</p>
...
<p class="meta">${esc(a.state)} Â· ${esc(a.origin)} proposal Â· ${esc(a.privacy)} Â· digest ${esc(short(a.inputDigest))} Â· ${a.retryCount}/${a.maxRetries} retries</p>
...
${cards(scoped(s.activity).slice(0, 300), (a) => `<article class="card"><h2>${esc(a.action)}</h2><p>${esc(a.summary)}</p><p class="meta">${esc(a.category)} Â· ${esc(a.outcome)} Â· ${esc(a.at)}${a.subjectRef ? ` Â· ${esc(short(a.subjectRef, 12))}` : ""}</p></article>`, "No activity recorded yet.")}`;
}
...
<form data-form="career"><label>Command<select name="command"><option value="init">init â€” create the private workspace and blank templates</option><option value="ingest">ingest â€” import one evidence file</option><option value="profile">profile â€” build the CareerProfile</option><option value="job:import">job:import â€” import one Job Posting you supply</option><option value="match">match â€” transparent scoring against a posting</option><option value="draft">draft â€” prepare owner-review materials</option><option value="export">export â€” write a local export bundle</option><option value="demo">demo â€” neutral synthetic walkthrough</option></select></label>
<label>Workflow root (absolute, not needed for demo)<input name="root" maxlength="4096"></label>
<label>Value â€” evidence file, posting file, Job Posting id, Match id, or export path<input name="value" maxlength="4096"></label>
<p class="hint">Run <code>init</code> first. Evidence files and Job Postings must then be placed inside <code>&lt;workflow root&gt;\private\input\</code> â€” the Career engine refuses any file outside that approved input root, and <code>init</code> writes blank templates there for you to fill in.</p>
...
${cards(s.imports, (r) => `<article class="card"><h2>${esc(r.platform)} Â· ${esc(r.state)}</h2>
<p>${r.items.length} file(s) Â· ${r.items.filter((x) => x.duplicate).length} duplicate(s) Â· ${r.items.reduce((n, x) => n + x.conversationCount, 0)} conversation(s) Â· ${r.importedConversationIds.length} imported</p>
<p class="meta">${esc(r.selectedRootRef)} Â· ${esc(r.createdAt)} Â· source files unchanged</p>
${r.warnings.length ? `<p class="warn">${esc(r.warnings.join(" "))}</p>` : ""}
<details><summary>Inventory</summary>${r.items.map((i) => `<p class="meta">${esc(i.relativePath)} â€” ${esc(i.classification)} Â· ${i.bytes} bytes Â· ${i.conversationCount} conversation(s)${i.duplicate ? " Â· duplicate" : ""}${i.digest ? ` Â· ${esc(short(i.digest))}` : ""}</p>`).join("")}</details>
...
<label>Provider<select name="providerId">${p.map((x) => `<option value="${esc(x.id)}" ${x.id === s.settings.providerId ? "selected" : ""}>${esc(x.id)} â€” ${esc(x.location)}${x.available ? ", ready" : ", unavailable"}</option>`).join("")}</select></label>
...
<label>Developer-agent bridge<select name="developerBridgeId"><option value="" ${s.settings.developerBridgeId ? "" : "selected"}>AION default â€” ${esc(model.developerBridge.displayName)}</option>${bridges.map((b) => `<option value="${esc(b.bridgeId)}" ${b.bridgeId === s.settings.developerBridgeId ? "selected" : ""}>${esc(b.displayName)}${b.available ? "" : " â€” unavailable"}</option>`).join("")}</select></label>
...
${bridges.length ? `<ul>${bridges.map((b) => `<li><b>${esc(b.displayName)}</b>${b.selected ? " Â· selected" : ""} â€” ${b.available ? "installed" : "unavailable"}${b.executable ? ` (<code>${esc(b.executable)}</code>${b.version ? `, ${esc(b.version)}` : ""})` : ""}<br><span class="meta">${esc(b.detail)}</span><br><span class="meta">Account: ${esc(b.account)} â€” ${esc(b.accountDetail)}</span>
${b.commands.map((c) => `<br><span class="meta">Exact ${esc(c.mode)} command: <code>${esc(c.executable)} ${esc(c.args.join(" "))}</code> â€” your instruction is written to standard input, never to this list.</span>`).join("")}</li>`).join("")}</ul>` : `<p class="empty">No developer-agent bridge was found.</p>`}
<div class="actions"><button data-do="developer-health">Check developer-agent account health</button></div></div>
<div class="card"><h2>Data locations</h2><p class="meta">Assistant state: <code>${esc(model.dataRoot)}</code> Â· exports and private backups: <code>${esc(model.exportRoot)}</code>. Both are inside the ignored private directory and are excluded from Git and from source backups.</p>
...
function render() {
  ...
  badge.textContent = provider?.location === "remote" ? `â— Remote provider selected (${provider.id})` : "â— Local-only";
...
document.addEventListener("click", async (event) => {
  ...
  if (!button) return;
  const { area: target, action, do: verb, id, state, enabled, value, workspace } = button.dataset;
  ...
  try {
    ...
    if (verb === "state-export") { download("aion-local-export.json", (await api("state.export")).export); toast("Complete local export written. It is plaintext â€” store it carefully."); return; }
...
document.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (kind === "verify") { await api("action.propose", { capabilityId: "aion.verify.run.v1", input: { operationId: d.operationId } }); toast("Verification proposed. Approve it in Approvals, then execute â€” AION runs it, not a model."); }
